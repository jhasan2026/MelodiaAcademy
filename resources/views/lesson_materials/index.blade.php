                                    ...
                                    <div class="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
                                        <div class="flex items-center justify-between gap-3">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-xs text-gray-300">
                                                    {{ $material->assignments?->count() ?? 0 }} item(s)
                                                </span>
                                            </div>

                                            <a href="{{ route('assignments.create', $material) }}"
                                               class="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold
                                                      ...
                                                      hover:bg-indigo-500/30 hover:scale-[1.01] transition">
                                                + Create
                                            </a>
                                        </div>

                                        @if(($material->assignments?->count() ?? 0) > 0)
                                            <div class="mt-3 space-y-2">
                                                @foreach($material->assignments->take(3) as $a)
                                                    <div class="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                                                        <div class="min-w-0">
                                                            ...
                                                            <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-300">
                                                                ...
                                                                @else
                                                                    <span class="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-gray-300">
                                                                        Draft
                                                                    </span>
                                                                @endif
                                                                ...
                                                                @if($a->due_at)
                                                                    <span class="rounded-full border border-amber-400/20 bg-amber-500/10 px-2 py-0.5 text-amber-200">
                                                                        Due {{ \Illuminate\Support\Carbon::parse($a->due_at)->format('M d, Y') }}
                                                                    </span>
                                                                @endif
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="flex shrink-0 items-center gap-2">
                                                            <a href="{{ route('assignments.edit', $a) }}"
                                                               class="rounded-lg border border-yellow-400/20 bg-yellow-500/10 px-3 py-1.5 text-xs font-semibold text-yellow-200 hover:bg-yellow-500/20 transition">
                                                                Manage
                                                            </a>

                                                            <a href="{{ route('assignments.submissions', $a) }}"
                                                               class="rounded-lg border border-cyan-400/20 bg-cyan-500/10 px-3 py-1.5 text-xs font-semibold text-cyan-200 hover:bg-cyan-500/20 transition">
                                                                Submissions
                                                            </a>
                                                        </div>
                                                    </div>
                                                ...
                                                @if($material->assignments->count() > 3)
                                                    <div class="pt-2 text-center">
                                                        <span class="text-xs text-gray-400">
                                                            Showing latest 3 assignments
                                                        </span>
                                                    </div>
                                                @endif
                                            </div>
                                        @else
                                            <div class="mt-3 rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-gray-300">
                                                No assignments yet. Create a quiz or recording task for this material.
                                            </div>
                                        @endif
                                    </div>
