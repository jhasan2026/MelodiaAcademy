    ...
    <section class="relative pt-36 pb-24">
        ...
        @if($user->role === 'student' | $user->role === 'instructor')
            <div class="absolute inset-x-0 top-0 z-0 h-72 overflow-hidden">
                ...
                >
                <div class="absolute inset-0 bg-gradient-to-b from-black/35 via-black/25 to-black/65"></div>
        ...
        <div class="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-8">
            {{-- Avatar --}}
            @if($user->role === 'student' | $user->role === 'instructor')
                <div class="flex items-center justify-center sm:justify-start mb-7">
                    <div class="relative">
                        ...
                        <img
                            src="{{ $profile && $profile->profile_pic ? asset($profile->profile_pic) : asset('images/default.jpg') }}"
            ...
            <div class="flex flex-col sm:flex-row max-sm:gap-5 items-center justify-between mb-7">
                <div class="text-center sm:text-left">
                    <h3 class="font-manrope font-bold text-4xl sm:text-[2.6rem] text-gray-900 drop-shadow-sm mb-1 tracking-tight">
                ...
                <div class="flex flex-col lg:flex-row max-lg:gap-5 items-center justify-between">
                    <div class="flex items-center gap-4">
                        <a
                            ...
                            class="py-3.5 px-6 rounded-full bg-indigo-600 text-white font-semibold text-base leading-7 shadow-lg shadow-indigo-600/20 transition-all duration-300 hover:bg-indigo-700 hover:shadow-indigo-700/25 active:scale-[0.99] ring-1 ring-white/10"
                        >
                            Edit Profile
                        </a>
                    </div>
                    ...
                    <div class="px-5 py-6 sm:px-8">
                        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-6">
                            <div class="rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-xs">
                                ...
                                <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $user->first_name }}</dd>
                            </div>

                            <div class="rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-xs">
                                <dt class="text-xs font-semibold uppercase tracking-wide text-gray-500">Last Name</dt>
                                <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $user->last_name }}</dd>
                            </div>
                            ...
                            @if($user->role === 'student')
                                <div class="rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-xs">
                                    ...
                                    <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $profile->gender }}</dd>
                                </div>

                                <div class="rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-xs">
                                    <dt class="text-xs font-semibold uppercase tracking-wide text-gray-500">Date of Birth</dt>
                                    <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $profile->date_of_birth }}</dd>
                                </div>

                            @elseif($user->role === 'instructor')
                                <div class="rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-xs sm:col-span-2">
                                    <dt class="text-xs font-semibold uppercase tracking-wide text-gray-500">Bio</dt>
                                    <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $profile->bio }}</dd>
                                </div>

                                <div class="rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-xs">
                                    <dt class="text-xs font-semibold uppercase tracking-wide text-gray-500">Specialization</dt>
                                    <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $profile->specialization }}</dd>
                                </div>

                                <div class="rounded-2xl border border-gray-200/70 bg-white px-5 py-4 shadow-xs">
                                    <dt class="text-xs font-semibold uppercase tracking-wide text-gray-500">Experience Years</dt>
                                    <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $profile->experience_years }}</dd>
                                </div>
                            @endif
                        </dl>
                    </div>
